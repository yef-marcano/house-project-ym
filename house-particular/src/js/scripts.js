...
function init() {
  ...
  light = new THREE.PointLight(0xffffff, 0, 448);
  ...
  mtlLoader.load("ProyectoBR.mtl", function (materials) {
    materials.preload();
    var objLoader = new THREE.OBJLoader();
    objLoader.setMaterials(materials);
    objLoader.setPath("src/models/");
    objLoader.load("ProyectoBR.obj", function (mesh) {
      mesh.traverse(function (node) {
        if (node instanceof THREE.Mesh) {
          node.castShadow = true;
          node.receiveShadow = true;

          //node.material = new THREE.MeshBasicMaterial({ wireframe: false });
